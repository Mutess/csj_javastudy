...
/*
		...
		==> 자바프로그램
		 ...
		 ==> 모든 개발자가 동일한 코딩을 하는 것
		
 */
import java.sql.*; // 컴파일 예외 => CheckedException = 예외처리가 필요하다.
import java.util.*;
public class EmpDAO {
	// 오라클 연결 객체
	...
	public EmpVO empDetailData (int empno) {
		...
		try {
			...
			ResultSet rs = ps.executeQuery();
			// rs의 있는 데이터를 VO에 담는다
			rs.next(); //데이터가 메모리에 출력된 첫번째 줄에 커서 위치이동
			vo.setEmpno(rs.getInt(1));
			vo.setEname(rs.getString(2));
			vo.setJob(rs.getString(3));
