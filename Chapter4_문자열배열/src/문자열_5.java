...
/*		String s = "Hello Java!!";
		//			01234567891011 = 총 12개
		System.out.println("원본 : "+ s);
		String s1 = s.substring(6);
		System.out.println(s1);
		String s2 = s.substring(0,5); //0 ~ 5번까지 잘라달라
		//						  -- 는 제외하고 잘라줌   그래서 (0, n-1) 값이 들어감 
		System.out.println(s2);
		String s3 = s.substring(6,10); // 가져올 문자열의 인덱스 번호 + 1를 해서 입력해야함
...
/*		for(String s:music) {
			if (s.length() > 37) {
				String ss = s.substring(0,34)+"...";      //뒤에 ...을 붙여야 하기에 -3을해서 34로 입력
...
/*		String s2 = addr.substring(0,17);
		System.out.println(s2); */
		
		String s = addr.substring(0, addr.lastIndexOf("지")); // = 0에서부터 lastIndexOf()는 뒤에서부터 "지"를 만나는 자리에서 -1을 해서 "지"전의 자리를 다 가지고 옴
		System.out.println(s);
		/*
		 * 인덱스가 있는 위치값을 가져오는 메소드
		 * lastIndexOf() //뒤에서부터 찾음
		 * indexOf()  // 앞에서부터 찾음
		 */
/*		String sss = "Hello Java";
		String s1 = sss.substring(sss.lastIndexOf("J"));
		System.out.println(s1); */
		
		String s2 = addr.substring(addr.lastIndexOf("서울")); //뒤에서부터 읽는데 시작값을 설정을 안했으니 뒤에서 부터 읽다가 "서울"을 만나면 처음 만난 "서울"부터 끝까지 읽음 
		System.out.println(s2);
		...
		String sss1 = str.replace("java", "Oracle");  //대소문자 구분해서 대체(replace)함
