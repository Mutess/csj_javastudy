 ...
 * 		---------- 확장을 할 수 없다. (있는 그대로 사용) => 상속을 내릴수 없다.
 * 				   상위클래스가 될 수 없다.
 * 		java.lang
 * 		java.util
 * 		java.io
 ...
 * 		객체지향의 3대 요소
 * 			1. 데이터 보호의 목적
 * 				캡슐화 : 변수를 은닉화 => 메소드를 통해서 접근이 가능하게 만들어줌
 * 					   -----------				 ---------
 * 						private					   public
 * 					=> 변수는 2가지 기능만 수행
 * 						메모리 저장 / 메모리에서 데이터읽기
 ...
 * 			
 * 			2. 재사용하는 목적 : 상속, 포함 ==> 클래스 크기 결정 ==> 형변환이 가능
 * 				class A 
 ...
 * 								   -----------------
 * 									메모리 할당시에 상속을 받은 클래스를 이용헤서 메모리 할당을 한다.
 * 									미완성 클래스 (메소드가 구현이 안된 상태)
 * 												------------------
 * 									=> 설계, 같은 기능 (처리를 프로그램에 맞게 할때)
 * 											버튼 클릭, 마우스 클릭...
 * 										ex)	게시판 : 글쓰기 (프로그램에 맞춰서 따로 구현되서 설계)
 * 					   B d = (B)c; => 명시적 형변환 (강제)
 * 					   ----------- 라이브러리에서 많이 사용
 ...
 *             	 ** 클래스(단일) / 인터페이스 (다중)
 *             --- 설계도 (구현이 안된 상태) => 미완성 클래스 (자신의 메모리 할당을 할 수 없다.)
 ...
 *             --------------------------------------------------
 *              멤버변수			인스턴스변수			상수형 변수 ==>  무조건 초기화를 해줘야 하기때문에 생성자를 필요로 함
 *              				정적변수
 *             --------------------------------------------------
 ...
 *                    	 구현한 메소드 목록
 *                    	 default void aaa(){}
 *                    		=> 컴파일러가 인식할때 public를 붙혀서 인식함
 *                    		=> public default void aaa(){} 이렇게
 *                    	 static void bbb(){}
 *                    		=> 컴파일러가 인식할때 public를 붙혀서 인식함
 *                    		=> public default void bbb(){} 이렇게
 *                  	----------------------------
 *                  
 *                  }
...
public class 정리_7장 {
	class A {
		int a = 10;
		String name = "홍길동";
		public A() {}  //기본생성자는 다른 생성자가 있을때 자동으로 생성이 되지 않기에 기본생성자 하나를 넣어줘야함
		public A (int a) {
			this.a = a;
		}
	}
...
//		}
	...
	public static void main(String[] args) {
		...
		list.add(""); //object는 모든 데이터형을 저장할수 있음
		list.add(1);
		list.add(new 정리_7장());
		...
		String s1 = "Hello";  
		String s2 = "Hello";  //s1이랑 s2는 같은 객체
		String s3 = new String("Hello");  //하지만 new를 사용하면 다른 메모리 공간에 생성이 되기 때문에 다른 객체가 되서 equals로 비교해야함
		if (s1 == s2) {
