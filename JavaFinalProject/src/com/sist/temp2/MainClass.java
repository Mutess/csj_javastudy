			...
			---------------------------------------------------------------
			 ...
			 => 변수 (70%) => 제어문, 연산자
			  ...
			  --------------------------------------------------------------
			 	정수형 
			 		 byte : 네트워크 통신, 파일 입출력, 웹
			 		 		-----------------------
			 		 		ASCII코드를 주로 사용 (char당 바이트 : 1byte)
			 		 		한글 => 멀티바이트 (char당 바이트 : 2byte) => Unicode
			 		 				=> UTF-8
			 		 		=> 사용범위 => -128 ~ 127
			 		 => 웹프로그래머 : 자바(80%), 오라클 (20%)
			 		 웹브라우저	<=======> 	 오라클
			  ...
			  = 식별자
			  	...
			  	3) 특수문자 사용 ( _ , $)
			  	4) 키워드(예약어)는 사용불가 (메소드명, 클래스명은 키워드가 아니다.)
			  	5) 변수에 공백이 없다.
			  	------------------------------ 오라클, 자바스크립트에서도 동일
			  	...
			  	=> 약속 사항
			  	  ...
			  	  삼항연산자 : 웹에서 자주 사용 => if ~ else를 줄여서 사용할때 사용
			  	  			=> 웹, 게임
			  	  	예) JSP + HTML
				  	  	<%
				  	  		if (curpage > 1) {
				  	  	   ...
				  	  	   반복문 : 
				  	  	   		  반복횟수가 지정된 경우 : for
				  	  	   		  	...
				  	  	   		  	형식) 
				  	  	   		  		...
				  	  	   		  		1-2-3-4
				  	  	   		  		  2-3-4
				  	  	   		  		  2-3-4 ==> 2번이 false가 되면 종료
				  	  	   		  		  
				  	  	   		  		for (데이터형 변수:배열 or 컬렉션) {
				  	  	   		  			 --------- 일반변수/클래스일수도 있다
				  	  	   		  			==> 향상된 for문 , for-each ==> 웹에서 출력
				  	  	   		  		}
