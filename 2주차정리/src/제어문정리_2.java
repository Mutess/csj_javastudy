 ...
 *        		}
 *        
 *        다중 조건문 : 조건이 많을 경우 해당되는 조건만 수행이 가능
 *        if (조건문) {
 *        	조건문이 true일때 수행하는 문장 ===> 종료
 *        	조건문이 false 일때
 *        }		|
 *        else if (조건문) {
 *        	조건문이 true일때 수행하는 문장 ===> 종료
 ...
 *        		문장3
 *        case 4:
 *        		문장4 ==============> 2일때 2,3,4 수행후 종료
 *        		break;
 *        case 5:
 *        		문장5
 *        
 *        switch() {  == switch문 안에서 같은 블록안에 같은 변수명을 사용할 수 없음
 *        	case 1:
 *        		int a = 10;
 ...
 * 	  1) for : 반복횟수가 지정된 경우
 *    2) while : 반복횟수가 없는 경우 
 *    	=> 파일 읽기, 데이터 베이스 (횟수를 모를떄)
 *    3) do~while : 한번 이상을 수행할 때 사용
 * 3. 반복 제어문
 * 	  ------------ 페이징
 */
import java.io.*;
import java.util.Scanner;
public class 제어문정리_2 {
	public static void main(String[] args) {
	/*	int i = 2;                  //=============만약 하나의 변수로만 사용하고 싶으면 switch 선언 전에 int a를 입력
		switch(i) {
		case 1: {  //=======================이렇게 블럭이 다르면 각각 다른 메모리에 저장됨
			int a = 10;
			System.out.println(a);
		}
			break;
		...
		int j = 0; 
		int rowSize = 10; //단지 10바퀴를 돌리기 위해 10을 줌// 10줄을 출력하기 위함
		int pagecnt = (page*rowSize)-rowSize; //start// page*10 ==> 2페이지일때 20-10 == pagecnt==> 10번부터 출력해준다는 뜻
		//				1      10   -  10  =0 이니까 첫페이지만 출력
		//				2      10   -  10  = 10 이니까 두번쨰 페이지 출력
		/*
		 * 시작 위치
		 * start 		end
		 *   0			 9 => 1page
