...
/*
			  ...
			  Error							Exception (예외처리의 최상위 클래스)
			  	...
			  	2) 예외처리의 종류
			  		...
			  		try {
			  			...
			  			=> 에러발생시 catch로 이동
			  		       ------------------
			  		       에러밑에 있는 문장은 수행하지 않는다.
			  		}catch(예외처리 클래스){
			  			try수행시 에러가 발생했을때 처리되는 문장
			  			여러개를 사용할수 있다.
			  			=> 순서가 존재(상속도)
			  		...
			  		= ***예외회피 : 예외 떠넘기기 (라이브러리가 떠넘김)
			  		=> 사용법
			  			메소드 선언뒤에 예상되는 에러를 선언
			  			=> 메소드 호출시에 처리할 수 있게 만든다.
			  				...
			  				선언된 예외클래스가 많은 경우
			  					...
			  					예) 
			  						...
			  						호출
			  						public void display()  ==> 오류나는 코드
			  						{
			  							try {
			  							}catch (IOException)
			  						}
			  						------------------------------호출
			  						public void display() ==> 이건 오류 안남
			  						{
			  							try {
			  							}catch (IOException)
			  							try {
		...
		java.lang : 자바 프로그램의 기본 라이브러리 (가장 많이 사용)
			...
			=> String
				...
				***4. 지정한 문자가 문자열에 몇번째에 있는지를 반환한다.
					...
					int indexOf (String s), int indexOf(char)
		...
		---------
			...
			=> 문자열 분해
				StringTokenizer
					=> counterToken() : 분해된 문자열의 갯수
					=> hasMoreToken() : 데이터가 존재할떄까지 => true
						while을 이용해서 주로 사용
					=> nextToken() : 실제 분해된 데이터를 읽을 경우에 주로 사용
			 ...
			 ***ArrayList			***HashSet			 ***HashMap => 구현된 클래스
			 ...
			 => put() : 저장시에 사용
			 => get(key) : 저장된 데이터 읽기
		1) 기본 생성자를 사용해서 되면 모든 데이터형은 Object
			...
			Map map = new HashMap();
				...
				Object get (key)
			Set set = new HashSet();
				add(Object) get (int index)
				=> 관리하기 어렵다.
				=> Object가 추가 되면 모든 데이터형을 첨부
					=> 각각의 다른 데이터형이 섞여 있으면 관리가 어려움 => 즉, for문 수행이 어렵다.
					=> 같은 데이터형을 첨부하는 것이 편리하다.
					=> Object리턴이면 반드시 형변환을 시킨다.
					=> 메소드에서 사용되는 모든 매개변수, 리터형을 통일화 (제네릭)
					=> 형변환이 필요없기 때문에 소스가 간결해짐
					=> 표준화 시킬수가 있음
					=> 클래스뒤에 <데이터형 : 클래스형으로만 사용>
					예) 
						문자열
